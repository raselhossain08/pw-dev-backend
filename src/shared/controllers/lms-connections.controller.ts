...
import { LMSConnectionsService } from './lms-connections.service';
...
export class LMSConnectionsController {
    constructor(
        private readonly lmsConnectionsService: LMSConnectionsService,
    ) { }
    ...
    @ApiBearerAuth('JWT-auth')
    @ApiOperation({ summary: 'Get complete course structure with all related entities' })
    @ApiResponse({ status: 200, description: 'Course with modules, lessons, assignments, quizzes' })
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth('JWT-auth')
    @ApiOperation({ summary: 'Get module with all lessons, assignments, and quizzes' })
    ...
    async getStudentProgress(
        ...
        return this.lmsConnectionsService.getStudentProgress(
            req.user.id,
            courseId,
        );
    }
    ...
    @UseGuards(JwtAuthGuard)
    @ApiBearerAuth('JWT-auth')
    @ApiOperation({ summary: 'Check if student is eligible for course certificate' })
